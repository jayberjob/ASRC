...
async function loadBoard({ silent = false } = {}) {
  ...
  try {
    ...
    console.error(error);
    $("#connectionState").hidden = false;
    $("#connectionState").textContent = "게시판 데이터를 불러오지 못했습니다. Supabase 연결 정보를 확인해주세요.";
...
if (!db && ["localhost", "127.0.0.1"].includes(location.hostname)) {
  posts = [{
    ...
    nickname: "박재훈",
    category: "정기런",
    title: "7월 정기런 코스 의견 주세요",
    content: "여름에도 부담 없이 달릴 수 있는 코스를 함께 정해보면 좋겠습니다.",
    likes: 4,
  ...
  comments = [{
    id: "preview-comment",
    opinion_id: "preview-post",
    author_name: "김민수",
    content: "야간 코스도 같이 검토해보면 좋을 것 같아요.",
    created_at: new Date().toISOString()
  }];
}
